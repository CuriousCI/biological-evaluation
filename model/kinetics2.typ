...
== Reactions rate laws 2

$
    regulation(reaction) =
    product_((entity, reaction, activator) space in space edges) [entity] / (K_A^(entity, reaction) + [entity])
    product_((entity, reaction, inhibitor) space in space edges) K_I^(entity, reaction) / (K_I^(entity, reaction) + [entity])
$
$
    rate(reaction) = regulation(reaction) dot.c
    k_reaction product_(edge = (entity, reaction, input) space in space edges) [entity]^stoichiometry(edge)
$ <mass-action-rule-2>
...
$
    rate(reaction) = [e] dot.c regulation(reaction) dot.c N / D
...
$
    N =
    k_reaction product_(edge = (entity, reaction, input) space in space edges) ([entity] / K_m^(entity, reaction))^stoichiometry(edge)
$
...
// Given a reaction $reaction in reactions$, its rate can be defined based on the
// reversibility of the reaction and the requirement of enzymes for the reaction.
