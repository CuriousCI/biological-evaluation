...
#definition[Biochemical network][
    ...
    $(entities, reactions, compartments, rcomponents, f_compartments)$ where
    ...
    - $compartments$ the set of compartments in which the physical entities of
        the network are located

    $
        & Q = { e union m | \
        & quad e : powerset(entities times roles_rentity) -> NN_1 and \
        & quad m in powerset(entities times roles_rmodifier) \
        & }
    $

    - $rcomponents : reactions -> Q$ is the function that defines the components
        of each reaction

    - $f_compartments : entities -> powerset(compartments)$ is the function that
        defines the compartments related to a physical entity

    // $
    //     rcomponents : reactions -> A
    // $
...
#definition[Problem definition closure][
    ...
    $network = (entities, reactions, compartments)$, let
    $network' = (entities', reactions', compartments')$ be the closure of
    $scope$ in the network $network$ where:

    $
        & forall entity, reaction \
        & quad entity in entities_interest and \
        & quad exists stoichiometry space (entity, output, stoichiometry) in rcomponents(reaction) \
        & quad quad => \
        & quad quad quad reaction in reactions'
    $
    ...
    $
        ...
        & quad
        exists stoichiometry space (entity, output, stoichiometry) in rcomponents(reaction_2) \
        & quad quad
        => \
    ...
    - $rcomponents' (reaction) = rcomponents (reaction)$ where
        $reaction in reactions'$

    - $f'_compartments (entity) = f_compartments (entity)$ where
        $entity in entities'$
]
